...
toptionVar="512 512 512 512 1 1 512 512 1 1"
...
tRangeFilePath=${RangeFilePath}/${applicationName}DotProduct
...
for ((i=$StartConfig; i<=$numOfConfig; i=i+$StepConfig  ))
	...
	build/X86/gem5.opt  -d ${OutputBasePath}/${applicationName}DotProduct/${OutPrefix}C${configNumberStr} configs/example/se.py    --rangeFileName=${RangeFilePathForConfig} ${Configs[$i]} -c ${BinaryVar} --options="${optionVar}"  &
