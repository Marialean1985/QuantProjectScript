...
toptionVar="512 512 2 2 1 1 1 1 1 1"
...
tRangeFilePath=${RangeFilePath}/${applicationName}Sobel
...
for (( i=$StartConfig; i<=$numOfConfig; i=i+$StepConfig  ))
	...
	build/X86/gem5.opt  -d ${OutputBasePath}/${applicationName}Sobel/${OutPrefix}C${configNumberStr} configs/example/se.py    --rangeFileName=${RangeFilePathForConfig} ${Configs[$i]} -c ${BinaryVar} --options="${optionVar}"  &
